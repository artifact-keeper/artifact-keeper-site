...
Upload the package archive directly to the Composer endpoint. The artifact is sent as raw zip bytes, and the package name and version are read from `composer.json` inside the archive.

```bash
curl -X PUT \
  -H "Content-Type: application/zip" \
  --data-binary @mycompany-my-package-1.0.0.zip \
  -u your-username:your-password \
  http://localhost:8080/composer/my-repo/api/packages
```

> **Note:** The package name and version are always extracted from the `composer.json` file inside the uploaded zip archive. They cannot be overridden by request parameters. If the `version` field is absent from `composer.json`, it defaults to `dev-main`. The endpoint returns JSON `{"status":"ok","package":"...","version":"...","sha256":"..."}` on success (HTTP 201).

### Automated Publishing with Git Tags

Create a script to automatically publish from Git tags. The script creates a zip archive containing `composer.json` and uploads it to Artifact Keeper.
...
> **Tip:** Ensure your `composer.json` includes the correct `"version"` field before tagging. The version in the uploaded archive is what Artifact Keeper uses — it cannot be overridden in the request.
    ...
    steps:
      ...
      - name: Publish to Artifact Keeper
        run: |
          curl -X PUT \
            -H "Content-Type: application/zip" \
            --data-binary "@${{ env.ARCHIVE }}" \
            -u "${{ secrets.ARTIFACT_KEEPER_USER }}:${{ secrets.ARTIFACT_KEEPER_PASSWORD }}" \
            ${{ secrets.ARTIFACT_KEEPER_URL }}/composer/my-repo/api/packages
```
  ...
  script:
    ...
    - |
      curl -X PUT \
        ...
        --data-binary "@$ARCHIVE" \
        -u "$ARTIFACT_KEEPER_USER:$ARTIFACT_KEEPER_PASSWORD" \
        $ARTIFACT_KEEPER_URL/composer/my-repo/api/packages
  only:
    - tags
```
