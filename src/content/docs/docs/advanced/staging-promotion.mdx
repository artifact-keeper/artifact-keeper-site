...
When promoting, Artifact Keeper evaluates up to three layers of checks against each artifact. If any check fails with a **Block** action, the promotion is rejected.

```text
Quality Gate ── pass ──> CVE Policy ── pass ──> License Policy ── pass ──> Promoted
```

### Quality Gates

[Quality gates](/docs/security/quality-gates) evaluate the artifact's [health score](/docs/security/health-scores) against configured thresholds. Gates can enforce minimum scores (overall, security, quality, metadata), maximum issue counts, and required check completion.

Quality gates are evaluated first. If a gate fails with `action: "block"`, promotion stops before CVE and license checks run.
