...
A few additional environment variables control OIDC provisioning and the authorization request itself. See the [environment variable reference](/docs/reference/environment) for full detail.

```bash
OIDC_MAP_GROUPS_TO_GROUPS=false  # Reflect OIDC group claims as local groups (default: false)
OIDC_AUTO_CREATE_USERS=true      # Auto-provision users on first OIDC login (default: true)
OIDC_PKCE_ENABLED=true           # Enable PKCE (S256) on the authorization request (default: true)
```
...
### Group-to-Group Membership Mapping

Separately from the role mapping above, a SAML provider can reflect its assertion `groups` attribute values as membership in local Artifact Keeper groups, rather than (or in addition to) mapping them to a role. This is opt-in per provider via `map_groups_to_groups` on the SAML provider config (`POST`/`PUT /api/v1/admin/sso/saml`), and defaults to `false` so existing providers keep their pre-existing behavior (groups only feed `SAML_ADMIN_GROUP`/`SAML_GROUP_ROLE_MAP`) on upgrade.

When enabled, groups named by the assertion are auto-created (tagged with `external_source = "saml"` and the provider id) and membership rows are kept in sync on every login: memberships are upserted for groups the assertion lists and pruned for groups it no longer lists, scoped to that provider so SAML-managed, OIDC-managed, and operator-managed groups never interfere with each other.

**SSO-synced groups reject manual member edits.** Once a group is owned by an identity provider (`external_source` is set), `POST`/`DELETE /api/v1/groups/:id/members` return `409 Conflict` telling the caller to change membership in the identity provider instead. Only operator-created groups (`external_source` is `null`) can have their membership edited directly through the API.
