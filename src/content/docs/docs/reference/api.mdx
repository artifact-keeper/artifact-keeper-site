...
`must_change_password` is `true` when the account must set a new password before doing anything else (for example after an admin-forced reset). Clients should route to a password-change flow rather than using the tokens for other calls.

**TOTP flow:** when the account has TOTP enabled, this endpoint does not return real tokens on the first call. It returns a pending challenge instead, with `access_token`, `refresh_token`, `must_change_password`, and `token_type` omitted:
...
Exchange `totp_token` together with the current TOTP code via `POST /auth/totp/verify` to receive the real tokens (same response shape as this endpoint).
...
- `q` (string): Search by key/name
- `type` (string, alias `repo_type`): Filter by repository type (hosted, remote, virtual)
- `format` (string): Filter by format (docker, maven, npm, etc.)
- `project` (uuid): Filter to repositories assigned to this project
...
There is no `artifact_count` or `size_bytes` field; storage usage is reported as `storage_used_bytes`.
...
There is no string `role` field; admin status is the boolean `is_admin`.
...
`password` is optional; when omitted, a password is auto-generated.
...
```json
{
  ...
  "generated_password": "Xk7#mPq2Rn9!vLc4"
}
```

`generated_password` is present only when `password` was omitted from the request.
...
Projects are a metadata grouping of repositories: a project owns zero or more repositories (`repositories.project_id`), and a permission grant on a project is inherited by every repository assigned to it. All project endpoints are admin-only.
...
`quota_bytes` is stored but not yet enforced.
...
`key` must be 1-128 characters of letters, digits, `.`, `_`, or `-`, must not start with a dot or hyphen, and must not contain consecutive dots.

**Response:** the created project object (same shape as one `items` entry above).

**Errors:** `409` when the project key already exists.
...
List membership grants on a project (admin only).
...
Grant a user or group access to every repository in the project (admin only).
...
`principal_type` is `user` or `group`. `actions` must be non-empty.

### DELETE /projects/:id/members

Revoke a membership grant (admin only).
  ...
  "items": [
    {
      ...
      "low_count": 12,
      "info_count": 0,
      "started_at": "2026-02-01T12:29:00Z",
      "completed_at": "2026-02-01T12:30:00Z",
      "created_at": "2026-02-01T12:28:00Z"
    }
  ],
  "total": 1
}
...
Severity counts are flat fields (`critical_count`, `high_count`, `medium_count`, `low_count`, `info_count`), not a nested `vulnerabilities` object. There is no top-level `scanned_at`; use `started_at`/`completed_at`/`created_at`.

### POST /security/scan

Trigger a security scan. Note the path is singular (`/security/scan`, not `/security/scans`).
...
`artifact_id` and `repository_id` are both optional (target either one artifact or every artifact in a repository). `bypass_dedup` skips the hash-based dedup short-circuit and is admin-only; non-admin callers get `403` if they set it.
...
```json
{
  ...
  "total_artifacts": 1523,
  "total_storage_bytes": 107374182400,
  "total_downloads": 8562,
  "total_users": 42,
  "active_peers": 2,
  "pending_sync_tasks": 0,
  "proxy_artifact_count": 314,
  "proxy_storage_bytes": 10737418240
}
```

`total_storage_bytes` already includes proxy-cached and OCI layer/config blob bytes; `proxy_storage_bytes` is a breakdown of that total, not a separate figure to add on top.
