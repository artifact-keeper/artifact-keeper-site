...
| `LOG_FORMAT` | No | `pretty` | Diagnostics stdout log format. Accepted values are `pretty` (human-readable multi-line) and `json` (one JSON object per line, for structured stdout collection). Unrecognized values fall back to `pretty`. |
| `LOG_PROBE_REQUESTS` | No | `false` | Log the per-request "Request completed" line for health/readiness/liveness probe endpoints (`/health`, `/healthz`, `/ready`, `/readyz`, `/livez`). Off by default so probe polling does not bury real request logs; a non-2xx probe response is always logged. Read once at first request. |
...
| `SETUP_PASSWORD_HINT` | No | - | Free-text hint shown alongside the setup lock, telling operators how to retrieve the generated admin password (e.g. a `kubectl exec` command or a link to internal documentation). Leave unset to keep the built-in Docker Compose default. Whitespace-only values are ignored. |
...
| `OIDC_MAP_GROUPS_TO_GROUPS` | No | `false` | Reflect OIDC group claims as local Artifact Keeper groups. Accepts `true`/`1` or `false`/`0`. |
| `OIDC_AUTO_CREATE_USERS` | No | `true` | Auto-provision users on first OIDC login. Accepts `true`/`1` or `false`/`0`. |
| `OIDC_PKCE_ENABLED` | No | `true` | Enable PKCE (S256) on the authorization request. Accepts `true`/`1` or `false`/`0`. |
...
| `STORAGE_KEY_SCHEME` | No | `repo-scoped` | Storage key scheme for path-addressed format objects (Maven/sbt) on shared cloud backends (S3/GCS/Azure): `repo-scoped` writes `{format}/{repository_id}/{path}` so keys can never collide across repositories; `flat` keeps the legacy `{format}/{path}` layout. Reads fall back to the flat key, so existing objects stay readable either way. Filesystem storage is unaffected (already isolated per repository). |
...
| `AUDIT_STREAM` | No | `off` | Emit every recorded audit action as a self-contained NDJSON record on stdout, so it can be shipped to a SIEM by collecting stdout without polling the admin API or reading Postgres. `off` emits nothing; `stdout` emits one NDJSON audit record per line, matching the published schema at `backend/schemas/audit-event.v1.schema.json`. Independent of `LOG_FORMAT`, though pairing with `LOG_FORMAT=json` gives fully structured stdout. See the [audit log export reference](/docs/security/audit-log/#exporting-audit-events) for the schema. |
