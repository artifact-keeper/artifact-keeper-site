...
3. **Generates secure credentials**: Creates random values for `JWT_SECRET`, the Meilisearch master key, the PostgreSQL password, and (if not provided) the admin password. Credentials are generated using `openssl rand` when available, falling back to `/dev/urandom`.
