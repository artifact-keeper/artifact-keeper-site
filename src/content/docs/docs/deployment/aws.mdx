...
graph TD
    ...
    subgraph EC2["☁️ EC2 Instance · Ubuntu 24.04"]
        direction TB
    ...
    Internet --> UFW --> NGINX
    NGINX -->|"/ → :3000"| FE
    NGINX -->|"/api · /health → :8080"| BE
    PG -.-> PG_DATA
    MS -.-> MS_DATA
    BE -.-> STORAGE
    TV -.-> TV_CACHE

    classDef ec2 fill:#ff9900,color:#fff,stroke:#ff9900,font-weight:bold
    classDef host fill:#232f3e,color:#fff,stroke:#232f3e
    classDef docker fill:#1d63ed,color:#fff,stroke:#1d63ed
    classDef app fill:#2ea44f,color:#fff,stroke:#2ea44f
    classDef data fill:#6f42c1,color:#fff,stroke:#6f42c1
    classDef ebs fill:#d73a49,color:#fff,stroke:#d73a49
    classDef internet fill:#0969da,color:#fff,stroke:#0969da,font-weight:bold

    class Internet internet
    class NGINX,UFW,F2B host
    class FE,BE app
    class PG,MS,TV data
    class PG_DATA,MS_DATA,STORAGE,TV_CACHE ebs
```

**Nginx** runs on the host for SSL termination (with optional Let's Encrypt). All application services run as Docker containers managed by Docker Compose. Data is persisted to `/data/` on a dedicated encrypted EBS gp3 volume.
...
- [Helm Chart Deployment](/docs/deployment/helm/) -- Kubernetes cluster deployment
