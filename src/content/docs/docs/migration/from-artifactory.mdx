...
curl -X POST https://registry.example.com/api/v1/migrations/connections \
  ...
  -d '{
    "name": "my-artifactory",
    "url": "https://artifactory.corp.example.com/artifactory",
...
#### Step 2: Test Connection and List Repositories
...
#### Step 3: Create and Start Migration Job
...
curl -X POST https://registry.example.com/api/v1/migrations \
  ...
  -d '{
    ...
    "job_type": "full",
    "config": {
      "include_repos": ["libs-release-local", "docker-local"],
...
**Internal Server Error when migrating from HTTP Artifactory**:
```
Error: 500 Internal Server Error
```
If your Artifactory instance uses plain HTTP (no TLS), the backend rejects the connection by default. Set `ALLOW_HTTP_INTEGRATIONS=1` in the backend environment to allow HTTP connections:
```yaml
# docker-compose.yml
services:
  backend:
    environment:
      ALLOW_HTTP_INTEGRATIONS: "true"
```
See [Environment Variables](/docs/reference/environment#network-security) for details.

**Certificate error when migrating from HTTPS Artifactory with internal CA**:
```
Error: error sending request ... certificate verify failed
```
If your Artifactory uses HTTPS with a certificate signed by an internal or private CA, mount the CA certificate into the container and set `CUSTOM_CA_CERT_PATH`:
```yaml
# docker-compose.yml
services:
  backend:
    environment:
      CUSTOM_CA_CERT_PATH: /etc/ssl/certs/internal-ca.pem
    volumes:
      - ./internal-ca.pem:/etc/ssl/certs/internal-ca.pem:ro
```
The PEM file can contain multiple certificates for CA bundles. See [Environment Variables](/docs/reference/environment#network-security) for details.
...
- Community support: [GitHub Discussions](https://github.com/artifact-keeper/artifact-keeper/discussions)
